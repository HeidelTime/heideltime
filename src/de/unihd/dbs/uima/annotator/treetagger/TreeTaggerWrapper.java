...
public class TreeTaggerWrapper extends JCasAnnotator_ImplBase {
	...
	/**
	 * uimacontext to make secondary initialize() method possible. -> programmatic, non-uima pipeline usage.
	 * 
	 * @author julian
	 *
	 */
	 ...
	 * 
	 * @param language
	 *                Language/parameter file to use for the TreeTagger
	 * @param treeTaggerHome
	 *                Path to the TreeTagger folder
	 * @param annotateTokens
	 *                Whether to annotate tokens
	 * @param annotateSentences
	 *                Whether to annotate sentences
	 * @param annotatePartOfSpeech
	 *                Whether to annotate POS tags
	 * @param improveGermanSentences
	 *                Whether to do improvements for german sentences
	 */
	public void initialize(Language language, String treeTaggerHome, Boolean annotateTokens, Boolean annotateSentences, Boolean annotatePartOfSpeech, Boolean improveGermanSentences,
			String cnTokPath) {
	...
	public void initialize(UimaContext aContext) {
		...
		if (!abbFileFlag || !parFileFlag || !tokScriptFlag) {
			LOG.error("Cannot find tree tagger (" + ttprops.rootPath + ttprops.fileSeparator + "cmd" + ttprops.fileSeparator + ttprops.tokScriptName + ")."
	...
	public void process(JCas jcas) throws AnalysisEngineProcessException {
		...
		/*
		 * if the annotate_partofspeech flag is set, annotate partofspeech and, if specified, also tag sentences based upon the partofspeech tags.
		 */
	 ...
	 * 
	 * @param jcas
	 *                JCas object supplied by the pipeline
	 */
	private void tokenize(JCas jcas) {
		...
		for (String s : tokenized) {
			...
			if ((!(s.equals("EMPTYLINE"))) && (jcas.getDocumentText().indexOf(s, tokenOffset) < 0)) {
				LOG.error("Tokenization was interrupted because the token \"" + s + "\" could not be found in the original text. The reason for this might be "
						+ "that the encoding of the document is not UTF-8. This token was skipped and " + "if it was part of a temporal expression, will not be extracted.");
				continue;
			}
	...
	private void doTreeTag(JCas jcas) {
		try {
			...
			AnnotationIndex<Token> ai = jcas.getAnnotationIndex(Token.type);
			List<String> tokenStrings = new ArrayList<>();
			List<Token> tokens = new ArrayList<>();
			for (FSIterator<Token> fsi = ai.iterator(); fsi.hasNext();) {
				Token token = fsi.next();
	...
	private void doTreeTagOld(JCas jcas) {
		...
		try {
			...
			AnnotationIndex<Token> toks = jcas.getAnnotationIndex(Token.type);
			for (FSIterator<Token> ai = toks.iterator(); ai.hasNext();) {
				Token t = ai.next();
				tokens.add(t);
	...
	private void improveFrenchSentences(JCas jcas) {
		...
		while (changes) {
			changes = false;
			FSIndex<Sentence> annoHeidelSentences = jcas.getAnnotationIndex(Sentence.type);
			for (FSIterator<Sentence> iterHeidelSent = annoHeidelSentences.iterator(); iterHeidelSent.hasNext();) {
				Sentence s1 = iterHeidelSent.next();

				if ((s1.getCoveredText().endsWith("av.")) || (s1.getCoveredText().endsWith("Av.")) || (s1.getCoveredText().endsWith("apr.")) || (s1.getCoveredText().endsWith("Apr."))
						|| (s1.getCoveredText().endsWith("avant.")) || (s1.getCoveredText().endsWith("Avant."))) {
					if (iterHeidelSent.hasNext()) {
						Sentence s2 = iterHeidelSent.next();
	...
	private void improveGermanSentences(JCas jcas) {
		/*
		 * these POS tag sequences will decide whether we want to merge two sentences that have (supposedly wrongfully) been split.
		 */
		...
		for (FSIterator<Sentence> sentIter = sentences.iterator(); sentIter.hasNext();) {
			if (thisSent == null) {
				thisSent = sentIter.next();
				continue;
			}
			...
			thisSent = sentIter.next();
			/*
			 * select the last two tokens within the previous sentence as well as the first of the current one and check for matches.
			 */
			Token penultimateToken = null, ultimateToken = null, firstToken = null;
			AnnotationIndex<Token> toks = jcas.getAnnotationIndex(Token.type);
			FSIterator<Token> tokIter = toks.subiterator(thisSent);
			if (tokIter.hasNext())
				firstToken = tokIter.next();

			tokIter = toks.subiterator(prevSent);
			while (tokIter.hasNext()) {
				penultimateToken = ultimateToken;
				ultimateToken = tokIter.next();
			}
			...
			for (String[] posRule : posRules) {
				/*
				 * either one of the pre-defined POS rules fit, or the first token's covered text begins with lower case characters.
				 */
				if ((penultimateToken.getPos() != null && penultimateToken.getPos().matches(posRule[0]) && ultimateToken.getPos() != null && ultimateToken.getPos().matches(posRule[1])
						&& firstToken.getPos() != null && firstToken.getPos().matches(posRule[2])) || firstToken.getCoveredText().matches("^[a-z/].*")) {
					/*
					 * check whether one of the previous candidate pairs already contains one of our sentences.
					 */
					...
					/*
					 * if one of the sentences was not already to be merged with another, create a new merge candidate set
					 */
					if (!candidateExisted) {
		...
		for (HashSet<Sentence> mergeCandidate : toMerge) {
			...
			int beginIndex = Integer.MAX_VALUE, endIndex = Integer.MIN_VALUE;
	...
	private void renumberSentences(JCas jcas) {
		AnnotationIndex<Sentence> sentences = jcas.getAnnotationIndex(Sentence.type);
		ArrayList<Sentence> sents = new ArrayList<>(sentences.size());
		for (FSIterator<Sentence> it = sentences.iterator(); it.hasNext();)
			sents.add(it.next());
		int i = 0;
